...
class Graph_Preprocessing_Layer(tf.keras.layers.Layer):
  ...
  def __call__(self, inp, training=None):
    inp_shape = tf.shape(inp)[-1:]
    collapsed_shape = tf.concat(([-1], inp_shape), axis=0)
    long_state = tf.reshape(inp, collapsed_shape) # (sample*N*T, 15)
    ego, surr1, surr2, surr3 = tf.split(long_state, [3, 4, 4, 4], axis=1)
    inp1 = tf.concat([ego, self.nn_e2e3(self.nn_e2e2(self.nn_e2e1(surr1)))], axis=1)
    inp2 = tf.concat([ego, self.nn_e2e3(self.nn_e2e2(self.nn_e2e1(surr2)))], axis=1)
    inp3 = tf.concat([ego, self.nn_e2e3(self.nn_e2e2(self.nn_e2e1(surr3)))], axis=1)
    out = self.nn_ve2v2(self.nn_ve2v1(inp1+inp2+inp3))
    expanded_shape = tf.concat((tf.shape(inp)[:-1], [self.feature_size]), axis=0)
